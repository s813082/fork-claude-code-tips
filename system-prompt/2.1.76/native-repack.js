...
function extractFromELF(binary) {
  ...
  return { bunOffsets, bunData: Buffer.concat([dataRegion, offsetsBytes, trailerBytes]), dataStart };
}
...
if (binary.format === "ELF") {
  ...
  const { bunData, bunOffsets, dataStart } = extractFromELF(binary);

  patchBunDataInPlace(bunData, bunOffsets, patchedCli);

  // bunData was constructed from overlay subsets via Buffer.concat, so
  // we need to write it back. The data region starts at dataStart in the overlay.
  // Overlay starts after the ELF content.
  const elfSize = binaryBuf.length - binary.overlay.length;
  const overlayOffset = elfSize + dataStart;
  // Copy the patched data region back (same size, bunOffsets.byteCount bytes)
